...
async function addHazard(){if(!user){alert('공유 위험 표시는 로그인 후 추가할 수 있습니다.');$('#profileDialog').showModal();return}const item={...makeEntry(),species:current.name,reporterId:user.uid};hazards=[item,...hazards.filter(x=>x.id!==item.id)];saveMemberFallback('hazards',hazards);renderHazards();location.hash='map';try{await setDoc(doc(db,'hazards',item.id),item);alert('공유 위험 표시에 추가했습니다.')}catch(err){alert('위험 표시는 이 기기에 추가되었고 서버 동기화는 나중에 다시 시도됩니다.')}}
async function removeHazard(id){const target=hazards.find(x=>x.id===id);if(!user||!target||target.reporterId!==user.uid)return;hazards=hazards.filter(x=>x.id!==id);saveMemberFallback('hazards',hazards);renderHazards();try{await deleteDoc(doc(db,'hazards',id));alert('위험 표시를 삭제했습니다.')}catch{alert('화면에서는 삭제했지만 서버 삭제는 나중에 다시 시도됩니다.')}}
const mergeEntries=(remote,local)=>[...remote,...local.filter(x=>!remote.some(y=>y.id===x.id))];
function startSync(){if(stopGuides){stopGuides();stopGuides=null}if(stopHazards){stopHazards();stopHazards=null}if(user){guides=JSON.parse(localStorage.getItem(memberKey('guides'))||'[]');hazards=JSON.parse(localStorage.getItem(memberKey('hazards'))||'[]');renderGuide();renderHazards();stopGuides=onSnapshot(query(collection(db,'users',user.uid,'guides'),orderBy('createdAt','desc')),snap=>{guides=mergeEntries(snap.docs.map(d=>d.data()),guides);saveMemberFallback('guides',guides);renderGuide()},()=>renderGuide());stopHazards=onSnapshot(query(collection(db,'hazards'),orderBy('createdAt','desc')),snap=>{hazards=mergeEntries(snap.docs.map(d=>d.data()),hazards);saveMemberFallback('hazards',hazards);renderHazards()},()=>renderHazards())}else{guides=guestGuides();hazards=guestHazards();renderGuide();renderHazards()}}
