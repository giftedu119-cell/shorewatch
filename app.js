...
const firebaseConfig=window.APP_CONFIG?.firebaseConfig; const fbApp=initializeApp(firebaseConfig); const auth=getAuth(fbApp); const db=getFirestore(fbApp); const provider=new GoogleAuthProvider();
...
async function saveGuide(){const item=makeEntry();if(user){if(item.image.length>850000)return alert('사진 용량이 커서 저장할 수 없습니다. 더 작은 사진으로 다시 올려 주세요.');await setDoc(doc(db,'users',user.uid,'guides',item.id),item)}else{guides.unshift(item);sessionStorage.setItem(guestKey,JSON.stringify(guides));renderGuide()}location.hash='guide'}
async function removeGuide(id){if(user)await deleteDoc(doc(db,'users',user.uid,'guides',id));else{guides=guides.filter(x=>x.id!==id);sessionStorage.setItem(guestKey,JSON.stringify(guides));renderGuide()}}
async function addHazard(){if(!user){$('#profileDialog').showModal();return}const item={...makeEntry(),species:current.name,reporterId:user.uid};await setDoc(doc(db,'hazards',item.id),item);location.hash='map'}
async function removeHazard(id){await deleteDoc(doc(db,'hazards',id))}
function startSync(){if(stopGuides){stopGuides();stopGuides=null}if(stopHazards){stopHazards();stopHazards=null}if(user){stopGuides=onSnapshot(query(collection(db,'users',user.uid,'guides'),orderBy('createdAt','desc')),snap=>{guides=snap.docs.map(d=>d.data());renderGuide()},err=>console.error(err));stopHazards=onSnapshot(query(collection(db,'hazards'),orderBy('createdAt','desc')),snap=>{hazards=snap.docs.map(d=>d.data());renderHazards()},err=>console.error(err))}else{guides=guestGuides();hazards=guestHazards();renderGuide();renderHazards()}}
...
$('#analyzeButton').onclick=async()=>{if(!imageData)return alert('먼저 생물 사진을 올려주세요.');try{$('#modelStatus').textContent='17종 사진 특징을 비교 분석 중입니다…';const p=await(await getModel()).predict($('#preview'));p.sort((a,b)=>b.probability-a.probability);const name=normal(p[0].className);current={name,info:SPECIES[name],ranking:p.slice(0,3)};$('#modelStatus').textContent='분석이 완료되었습니다.';showResult()}catch{$('#modelStatus').textContent='분석 모델 연결에 실패했습니다. 인터넷 연결 후 다시 시도해 주세요.'}};
// 일부 브라우저에서 대화상자가 늦게 만들어지는 경우에도 앱이 멈추지 않도록 안전하게 연결합니다.
function bindAccountControls(){
  $('#profileButton')?.addEventListener('click',()=>$('#profileDialog')?.showModal());
  $('#googleLogin')?.addEventListener('click',async()=>{try{await enablePersistentLogin();await signInWithRedirect(auth,provider)}catch(err){alert(`로그인에 실패했습니다: ${err.code||err.message}`)}});
...
enablePersistentLogin().catch(console.error).finally(async()=>{
  try { await getRedirectResult(auth); } catch (err) { console.error('redirect-auth',err); }
  const reflectUser=next=>{user=next||auth.currentUser;updateAccount();startSync();if(user)$('#profileDialog').close()};
  onAuthStateChanged(auth,reflectUser);
  if(auth.currentUser) reflectUser(auth.currentUser);
});
